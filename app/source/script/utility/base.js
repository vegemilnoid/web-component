...
APP_GLOBAL_FUNCTION[`${APP_GLOBAL_CONFIG.ID.toLowerCase()}Base`] = {
  import: self => {
    ...
    /**
     * Parses the provided attributes string and converts it into an object with key-value pairs.
     *
     * This method takes a string typically containing attributes and their values (e.g., from
     * an HTML element or similar structure), processes it, and returns an object where the
     * keys are attribute names and the values are their corresponding values.
     *
     * @param {string} attributesString - The string containing attributes to parse. It should
     *                                     be a properly formatted attributes string.
     * @returns {Object} An object representing the parsed attributes, where keys are attribute
     *                   names and values are their corresponding values. If the input is invalid
     *                   or empty, it may return an empty object.
     * @deprecated Use `bindAttribute` instead.
     */
    self.parseAttributes = attributes => {
      return attributes.map(attribute => {
        ...
        return {
          ...
          bind: (name, value) => {
            if(/\./.test(name)) {
              Object.assign(self.view, toNestedObject(name, toValue(value)));
            }
            else {
              const key = toCamelCase(name);
              self.view[key] = _.isBoolean(self.view[key]) ? value === '' : toValue(value);
            }
          }
        };
      });
